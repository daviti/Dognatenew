(function($) {
    ...
    $(document).ready(function() {
                ...
                if ($(document).find(".playerYoutube-bg").hasClass("playerYoutube-bg")) {
                    $(document).find(".playerYoutube-bg").each(function(key, data) {
                        new YT.Player($(data).attr("id"), {
                            ...
                            playerVars: {'autoplay': 0, 'controls': 0, 'showinfo': 0, 'loop': 1, 'playlist': 'kn-1D5z3-Cs', 'wmode': 'transparent'},
                            events: {
        ...
        $(document).delegate(".submit-contact", "click", function() {
            ...
            var check = true;
            if ($(contact).find(".name").val() === "") {
                $(contact).find(".name").addClass("error");
                $(contact).find(".name").attr("placeholder", "Name is Empty");
                $(contact).find(".name").focus();
                check = false;
            } else {
                $(contact).find(".name").removeClass("error");
            }

            if ($(contact).find('.email').val() === "") {
                $(contact).find(".email").addClass("error");
                $(contact).find(".email").attr("placeholder", "E-mail is Empty");
                $(contact).find('.email').focus();
                check = false;
            } else {
                if (isValidEmailAddress($(contact).find('.email').val()) === false) {
                    $(contact).find('.email').val("");
                    $(contact).find(".email").addClass("error");
                    $(contact).find(".email").attr("placeholder", "E-mail is Incorrect");
                    $(contact).find('.email').focus();
                    check = false;
                } else {
                    $(contact).find(".email").removeClass("error");
                }
            }

            if ($(contact).find('.description').val().length < 8) {
                $(contact).find(".description").val("");
                $(contact).find(".description").addClass("error");
                $(contact).find(".description").attr("placeholder", "Description Should Message Than 8 Character.");
                $(contact).find(".description").focus();
                check = false;
            } else {
                $(contact).find(".description").removeClass("error");
            }
